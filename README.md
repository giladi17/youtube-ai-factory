YouTube AI Factory
An autonomous, event-driven video production pipeline that runs entirely on AWS EKS. Given a schedule (or a single webhook call), the system researches trending tech topics, writes a script, generates an AI avatar video, edits it with FFmpeg, and publishes the finished result to YouTube — with zero human intervention.
...
The c5.2xlarge node is provisioned by Karpenter only for Phase 3 and terminated within ~90 seconds of the FFmpeg job completing.
